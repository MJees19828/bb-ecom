...
const productdetails = (props) => {
  ...
  const { updateCartItemCount, removeToCart, addToCart ,cartItems,closedProductDetails,
    selectedProduct, } = useContext(ShopContext);
  // Set selectedProduct to 0
const productId = selectedProduct || 0 ;

const product = PRODUCTS.find((p)=> p.id === productId) || PRODUCTS1.find((p)=>p.id === product);
  ...
  return <>
   ...
   <div className="d-flex ">
    ...
    <div className="col-6 card p-3">
     ...
     <p className="p-4">Was<strike className='text-danger'>{product.price * 2 } </strike>&nbsp; Now <span className='price'>${product.price}</span></p>
      <p className="mb-3">Lorem ipsum dolor sit amet consectetur adipisicing elit. Assumenda distinctio numquam ipsam quisquam neque voluptatem!</p>
     </div>
